...
import { z } from 'zod';
import { ajouterPiece, modifierPiece, supprimerPiece } from '../services/pieces.service';
...
export async function ajouter(req: Request, res: Response): Promise<void> {
  ...
  const donnees = req.body as PieceInput;
  const resultat = await ajouterPiece(idBailleur, id, donnees);
...
export async function modifier(req: Request, res: Response): Promise<void> {
  ...
  const idBailleur = req.utilisateur!.sub;
  const donnees = req.body as PieceInput;
  const resultat = await modifierPiece(idBailleur, id, donnees);
...
export async function supprimer(req: Request, res: Response): Promise<void> {
  ...
  const idBailleur = req.utilisateur!.sub;
  const resultat = await supprimerPiece(idBailleur, id);
